...
# ==========================================
# 2. HÀM TRÍCH XUẤT ĐẶC TRƯNG TỪ 1 BỨC ẢNH
# ==========================================
...
# ==========================================
# 4. PHÂN TÍCH VÀ TRỰC QUAN HÓA (EDA)
# ==========================================
def plot_and_export_eda(df, output_pdf="EDA_Report_VongMac.pdf", output_img_dir="EDA_Images"):
    print(f"\n--- BẮT ĐẦU VẼ VÀ XUẤT BÁO CÁO EDA ---")
    df = df.dropna().copy()
    
    # Tạo thư mục chứa ảnh lẻ (nếu chưa có thì tự động tạo mới)
    os.makedirs(output_img_dir, exist_ok=True)

    # Mở một phiên làm việc với file PDF
    with PdfPages(output_pdf) as pdf:
        
        # ---------------------------------------------------------
        # TRANG 1 (FIGURE 1): ĐẶC TÍNH QUANG HỌC
        # ---------------------------------------------------------
        ...
        # LƯU TRANG 1 VÀO PDF VÀ PNG
        pdf.savefig(fig1) # Thêm trang vào PDF
        fig1.savefig(os.path.join(output_img_dir, "01_Optical_Features.png"), dpi=150, bbox_inches='tight')
        plt.close(fig1) # TẮT BIỂU ĐỒ ĐỂ GIẢI PHÓNG RAM (Rất quan trọng)


        # ---------------------------------------------------------
        # TRANG 2 (FIGURE 2): THỐNG KÊ KÍCH THƯỚC
        # ---------------------------------------------------------
    ...
    print(f"🖼️ Ảnh rời (PNG): Thư mục '{output_img_dir}/'")
# ==========================================
# THỰC THI CHƯƠNG TRÌNH
# ==========================================
if __name__ == '__main__':
    # Bỏ comment 2 dòng dưới đây để chạy
