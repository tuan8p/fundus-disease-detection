...
def predict_labels(raw_output: torch.Tensor, coef: list = None) -> torch.Tensor:
    preds = raw_output.view(-1)
    if coef is None:
        coef = [0.5, 1.5, 2.5, 3.5]
    
    coef_tensor = torch.tensor(coef, device=preds.device)
    preds = torch.bucketize(preds.contiguous(), coef_tensor)
    return preds.long()